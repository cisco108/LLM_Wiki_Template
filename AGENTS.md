...
11. **Decision rationale:** when a page documents a framework, library, architecture, or pattern choice that wasn't the obvious/standard default — a real decision with tradeoffs — include a `## Why` section explaining the reasoning, not just what was chosen. Standard/unremarkable choices don't need this.
12. **Visual results:** when a source project produces visual output (plots, screenshots, diagrams, rendered UI, generated images), save the image into `wiki/assets/<project-slug>/` and embed it on the relevant page (`![alt](../assets/<project-slug>/file.png)` per `link_style`).
